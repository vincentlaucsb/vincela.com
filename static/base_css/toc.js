/* Requires:
 * spoiler.js and spoiler.css so the table of contents can be collapsible
*/
...
function menuFade(scroll_pos) {
    ...
    if (scroll_pos > TOC_REL_YPOS) { // Change style if we have scrolled past the desired point
        if (toc_fixed == false) { // Make sure the style has not yet been changed
...
function menuPosition(scroll_pos) {
    ...
    if (window.innerWidth < 900) {
        ...
        /* Code to hide the table of contents
        addClass(toc_list,"spoiler-spoiled");
        removeClass(toc_list,"spoiler");
        toc_list_button.innerHTML='Hide';*/
    }
}
    ...
    if (!ticking) {
        window.requestAnimationFrame(function() {
            if (ev.type == 'scroll') {
                ...
                toc_fixed = false; // Changing this variable allows menuFade to do something
